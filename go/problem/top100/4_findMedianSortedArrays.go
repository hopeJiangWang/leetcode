...
func FindMedianSortedArrays3(nums1, nums2 []int) float64 {
	...
	if (len1 + len2) % 2 == 0 {
		return float64(res[(len1 + len2)/2] + res[(len1 + len2)/2-1]) / 2.0
	} else {
		return float64(res[(len1 + len2)/2])
	}
}

func FindMedianSortedArrays4(nums1, nums2 []int) float64 {
	/*
...
func findKthNumber(nums1, nums2 []int, k int) int {
	...
	/*
		分别找两个数组的第k/2-1个数，取其中的较小值，更新k值（选中数组的前面的数据更小）
		直到一个数组到了结尾 或者 k==1，取较小值
	*/
	for {
		if index1 == len(nums1) {
